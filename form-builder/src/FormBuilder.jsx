...
const FormBuilder = () => {
  ...
  useEffect(() => {
    ...
    if (location.pathname === '/form/new/edit') {
      // This is a new form being created, don't fetch from backend
    ...
    if (id) {
      if (location.pathname.endsWith('/edit')) {
        ...
        if (id === 'new') {
          ...
          // Existing form, fetch from backend
  ...
  const handleNextToBuilder = async () => {
    ...
    // with the draft form in state
    setCurrentForm(draftForm);
    setEditingFormId('new');
  ...
  const handleSaveForm = async () => {
    ...
    try {
      if (editingFormId && editingFormId !== 'new') {
        ...
        // Creating a new form
        const created = await createForm(currentForm);
        setEditingFormId(created.id); // Update the ID to the newly created form
      }
      await loadForms();
      navigate('/');
    } catch (e) {
  ...
  if ((id && location.pathname.endsWith('/edit')) || location.pathname === '/form/new/edit') {
    return (
      <FormBuilderView
